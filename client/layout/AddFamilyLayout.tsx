...
import membersIcon from "@/assets/icons/members-icon/familyMember.png";
...
const notUserSVG = ({ active }: { active: boolean }) => (
  <svg
    ...
    xmlns="http://www.w3.org/2000/svg"
  >
    <g clip-path="url(#clip0_765_11505)">
      <path
        fill={active ? "#253E96" : "#7D7D7D"}
...
// ✅ Make sure the second tab has a unique route
const validServiceRoutes = [
  "/add-family-member",
  "/add-family-member/no-profile",
];

const AddFamilyLayout: React.FC = () => {
  ...
  // ✅ Sync active tab with route (when user navigates manually)
  useEffect(() => {
    if (validServiceRoutes.includes(location.pathname)) {
      setActiveTab(location.pathname);
    }
  }, [location.pathname]);

  // Check if route matches parent
  const isValidServiceRoute = validServiceRoutes.includes(location.pathname);

  return (
    <div className="min-h-screen bg-white lg:bg-[#F2F2F2] mx-auto">
      {/* Sub header */}
      ...
      <div
        ...
        className="flex items-center gap-4  lg:hidden p-3"
      >
      ...
      {/* Services tab */}
      {isValidServiceRoute && (
        <div className="max-w-7xl mx-auto   pt-5 flex flex-col lg:flex-row lg:flex-wrap lg:gap-x-4 lg:pl-6">
          {servicesTab.map((tab, index) => {
            ...
            return (
              <div
                ...
                className={`flex flex-col lg:flex-row lg:items-center gap-4 h-auto lg:h-[90px] mx-3 lg:mx-0 p-3 lg:p-0 lg:px-6 py-4 
        rounded-xl bg-[#F0F4FC] mb-4 lg:mb-0 lg:bg-white cursor-pointer transition hover:border-b-4 hover:border-primaryColor
        ${
          isActive
            ? "border-b-4 border-primaryColor text-primaryColor shadow-sm"
            : "hover:border-b-4 hover:border-primaryColor text-[#7D7D7D]"
        }`}
              >
